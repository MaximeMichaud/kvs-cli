...
class SystemHelpExamplesTest extends TestCase
{
    ...
    public function testCompletionDocumentationIncludesInstallOptionAndCurrentReloadHints(): void
    {
        $doc = file_get_contents(dirname(__DIR__) . '/docs/commands/completion.md');
        $this->assertIsString($doc);

        $this->assertStringContainsString('`-i, --install`', $doc);
        $this->assertStringContainsString('kvs completion --install', $doc);
        $this->assertStringContainsString('kvs completion bash --install', $doc);
        $this->assertStringContainsString('kvs completion zsh --install', $doc);
        $this->assertStringContainsString('kvs completion fish --install', $doc);
        $this->assertStringContainsString('source ~/.bash_completion', $doc);
        $this->assertStringContainsString('source ~/.zshrc', $doc);
        $this->assertStringContainsString('source ~/.config/fish/completions/kvs.fish', $doc);
    }
    ...
    public function testMigrateToDockerDocumentationUsesCompleteNonInteractiveExamples(): void
    {
        ...
        $this->assertStringContainsString('kvs migrate:import backup.tar.zst -d newdomain.com -e admin@newdomain.com', $doc);
        ...
        $this->assertStringNotContainsString('kvs migrate:import backup.tar.zst -d newdomain.com' . PHP_EOL, $doc);
    }

    public function testMigratePackageDocumentationUsesActualMetadataSurface(): void
    {
        $doc = file_get_contents(dirname(__DIR__) . '/docs/commands/migrate_package.md');
        $this->assertIsString($doc);

        $this->assertStringContainsString('Package metadata (version, paths, sizes)', $doc);
        $this->assertStringNotContainsString('Package metadata (version, paths, checksums)', $doc);
    }
    ...
    public function testAlbumAndUserDocumentationUsesCurrentActionsAliasesAndFilters(): void
    {
        ...
        $purgeDoc = file_get_contents($docsRoot . '/commands/user_purge.md');
        $this->assertIsString($purgeDoc);
        $this->assertStringContainsString('kvs user:purge [options]', $purgeDoc);
        $this->assertStringContainsString('dry-run by default', $purgeDoc);
        $this->assertStringContainsString('| `--removal-requested` | - | Match users who requested account deletion |', $purgeDoc);
        $this->assertStringContainsString('| `--confirm` | - | Actually delete matched users |', $purgeDoc);
        $this->assertStringContainsString('| `-y, --yes` | - | Skip confirmation prompt when `--confirm` is used |', $purgeDoc);
        $this->assertStringContainsString('kvs user:purge --removal-requested --no-content', $purgeDoc);
        $this->assertStringContainsString('kvs user:purge --no-content --limit=25', $purgeDoc);
        $this->assertStringContainsString('- `kvs user:cleanup`', $purgeDoc);
        $this->assertStringNotContainsString('kvs user:purge <status>', $purgeDoc);
        $this->assertStringNotContainsString('--dry-run', $purgeDoc);
        $this->assertStringNotContainsString('--force', $purgeDoc);
    }
