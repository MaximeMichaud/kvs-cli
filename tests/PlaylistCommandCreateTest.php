...
class PlaylistCommandCreateTest extends TestCase
{
    ...
    public function testPlaylistCreateRejectsDecimalUserIdBeforeLookup(): void
    {
        ...
        $this->assertStringContainsString('Invalid User ID', $tester->getDisplay());
        $this->assertSame(0, (int) $db->query('SELECT COUNT(*) FROM ktvs_playlists')->fetchColumn());
    }

    public function testPlaylistCreateRejectsScientificNotationUserIdBeforeLookup(): void
    {
        $db = $this->createPlaylistCreateDatabase();
        $tester = new CommandTester($this->createPlaylistCreateCommand($db));
    ...
    private function createPlaylistCreateDatabase(): PDO
    {
        $db = new PDO('sqlite::memory:');
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $db->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
