...
class CategoryGroupCommandTest extends TestCase
{
    ...
    public function testListShowsGroupsWithCategoryCount(): void
    {
        ...
        $this->assertStringContainsString('Categories amount', $output);
    }
    ...
    public function testListSearchesAdminFieldsBeyondTitle(): void
    {
        $exitCode = $this->tester->execute(['action' => 'list', '--search' => 'featured']);
        $output = $this->tester->getDisplay();
    ...
    public function testListAdminFieldAndUsageFilters(): void
    {
        $exitCode = $this->tester->execute(['action' => 'list', '--field-filter' => 'filled/screenshot1']);
        $output = $this->tester->getDisplay();
        ...
        $exitCode = $this->tester->execute(['action' => 'list', '--usage' => 'notused/categories']);
        $output = $this->tester->getDisplay();
        ...
        $exitCode = $this->tester->execute(['action' => 'list', '--used' => true]);
        $output = $this->tester->getDisplay();
    ...
    public function testListIdsFormatUsesCategoryGroupId(): void
    {
        $exitCode = $this->tester->execute(['action' => 'list', '--format' => 'ids']);

        $this->assertSame(0, $exitCode);
        $this->assertSame('2 1', trim($this->tester->getDisplay()));
    }
    ...
    public function testShowJsonFields(): void
    {
        $exitCode = $this->tester->execute([
            'action' => 'show',
            'id' => '1',
            '--format' => 'json',
            '--fields' => 'category_group_id,title,categories_amount,custom1,screenshot1',
        ]);

        $this->assertSame(0, $exitCode);
        $data = json_decode($this->tester->getDisplay(), true, flags: JSON_THROW_ON_ERROR);
        $this->assertSame([
            [
                'category_group_id' => 1,
                'title' => 'Genres',
    ...
    public function testCreateInactiveWithExplicitFields(): void
    {
        ...
        $this->assertSame('linear', $row['custom1']);
        $this->assertSame('broadcast', $row['custom2']);
        $this->assertSame('archive', $row['custom3']);
        $this->assertAuditLogExists(100, (int) $row['category_group_id'], '');
    }

    public function testCreateStoresScreenshotFile(): void
    {
        $image = $this->createTinyPng('category-group.png');
        $exitCode = $this->tester->execute([
            'action' => 'create',
            'id' => 'With Avatar',
            '--screenshot1' => $image,
        ...
        $this->assertSame(0, $exitCode);
        $row = $this->fetchGroupByTitle('With Avatar');
        $this->assertNotNull($row);
        $this->assertSame('s1_category-group.png', $row['screenshot1']);
        $this->assertFileExists(
            $this->kvsPath . '/contents/categories/groups/' . $row['category_group_id'] . '/s1_category-group.png'
        );
    }
    ...
    public function testUpdateRename(): void
    {
        ...
        $this->assertAuditLogExists(150, (int) $row['category_group_id'], 'title, sort_id');
    }

    public function testUpdateCanClearCustomAndScreenshotFields(): void
    {
        $groupDir = $this->kvsPath . '/contents/categories/groups/1';
        mkdir($groupDir, 0777, true);
        file_put_contents($groupDir . '/s1_genres.jpg', 'old image');
        ...
        $this->assertNotNull($row);
        $this->assertSame('', $row['custom1']);
        $this->assertSame('', $row['screenshot1']);
        $this->assertFileDoesNotExist($groupDir . '/s1_genres.jpg');
        $this->assertAuditLogExists(150, 1, 'custom1, screenshot1');
    }

    public function testUpdateReplacingScreenshotWithSameFilenameKeepsNewFile(): void
    {
        $groupDir = $this->kvsPath . '/contents/categories/groups/1';
        mkdir($groupDir, 0777, true);
        $target = $groupDir . '/s1_genres.jpg';
        file_put_contents($target, 'old image');
        $image = $this->createTinyPng('genres.jpg');
        ...
        $row = $this->fetchGroupByTitle('Genres');
        $this->assertNotNull($row);
        $this->assertSame('s1_genres.jpg', $row['screenshot1']);
        $this->assertFileExists($target);
        $this->assertSame(file_get_contents($image), file_get_contents($target));
    }
    ...
    public function testDeleteNonNumericId(): void
    {
        ...
        $this->assertSame(1, $exitCode);
        $this->assertStringContainsString('Invalid Category group ID', $this->tester->getDisplay());
    }
    ...
    public function testDeleteGroupWithCategoriesDetachesThemWhenConfirmed(): void
    {
        ...
        $categoryCount = $this->db->query('SELECT COUNT(*) FROM ' . TestHelper::table('categories'))->fetchColumn();
        $this->assertSame(3, (int) $categoryCount);
    }
    ...
    public function testUpdateRejectsNonNumericId(): void
    {
        ...
        $this->assertSame(1, $exitCode);
        $this->assertStringContainsString('Invalid Category group ID', $output);
    ...
    public function testCreateDedupesGeneratedDir(): void
    {
        ...
        $row = $this->fetchGroupByTitle('Genres!');
        $this->assertNotNull($row);
        $this->assertSame('genres2', $row['dir']);
    }
    ...
    private function assertAuditLogExists(int $actionId, int $objectId, string $details): void
    {
        $stmt = $this->db->prepare(
            ...
            ' AND object_type_id = 7 AND action_details = :action_details'
        );
        $stmt->execute([
            'username' => 'kvs-cli',
            'action_id' => $actionId,
    ...
    private function createDatabase(): PDO
    {
        ...
        $db->exec(
            ...
            'status_id INTEGER, external_id TEXT, screenshot1 TEXT, screenshot2 TEXT, ' .
            'custom1 TEXT, custom2 TEXT, custom3 TEXT, sort_id INTEGER, added_date TEXT)'
        );
        $db->exec(
        ...
        $db->exec(
            ...
            ' (category_group_id, title, dir, description, status_id, external_id, ' .
            'screenshot1, screenshot2, custom1, custom2, custom3, sort_id, added_date) VALUES ' .
            "(1, 'Genres', 'genres', 'Movie genres', 1, '', 's1_genres.jpg', '', 'featured', '', '', 0, '2026-01-01 10:00:00'), " .
